...
[[dImperative Programming]]とは、Function paradigmにCellの概念を加えたもの。
...
オブジェクト指向におけるオブジェクトを、
自ら判断し処理できる機能を持ったエージェントと呼ばれるモジュールに置き換えたもの。
...
- [[http://en.wikipedia.org/wiki/Agent-oriented_programming][Agent-oriented programming - Wikipedia
